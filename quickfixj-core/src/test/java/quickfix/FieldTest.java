...
public class FieldTest {
  ...
  @Test
  public void testFieldCalculationsWithUTF8Charset()
  ...
  public void testFieldhashCode() throws Exception {
    ...
    LocalDateTime date = LocalDateTime.now();
    assertEqualsAndHash(new UtcDateOnlyField(11, date.toLocalDate()),
                        new UtcDateOnlyField(11, date.toLocalDate()));
    assertEqualsAndHash(new UtcTimeOnlyField(11, date.toLocalTime()),
                        new UtcTimeOnlyField(11, date.toLocalTime()));
    assertEqualsAndHash(new UtcTimeStampField(11, date),
                        new UtcTimeStampField(11, date));
  }

  // QFJ-881
  @Test
  public void testMultipleStringValue() throws Exception {
    assertEquals(FieldType.MULTIPLESTRINGVALUE,
                 FieldType.fromName("notused", "MULTIPLESTRINGVALUE"));
    assertEquals(FieldType.MULTIPLEVALUESTRING,
                 FieldType.fromName("notused", "MULTIPLEVALUESTRING"));

    MarketDataIncrementalRefresh md = new MarketDataIncrementalRefresh();
    MarketDataIncrementalRefresh.NoMDEntries value =
  ...
  public void testMultipleCharValue() throws Exception {
    assertEquals(FieldType.MULTIPLECHARVALUE,
                 FieldType.fromName("notused", "MULTIPLECHARVALUE"));
  ...
  private void assertEqualsAndHash(Field<?> field1, Field<?> field2) {
    ...
    assertEquals("fields hashcode not equal", field1.hashCode(),
                 field2.hashCode());
  }
}
