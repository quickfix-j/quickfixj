...
| `DefaultApplVerID` | Required only for FIXT 1.1 (and newer). Ignored for earlier transport versions. Specifies the default application version ID for the session. This can either be the `ApplVerID` enum (see the `ApplVerID` field) or the `beginString` for the default version. Examples:<br>```<br># Enum. FIX 5.0 over FIXT 1.1<br>DefaultApplVerID=7<br><br># BeginString: FIX 5.0 over FIXT 1.1<br>DefaultApplVerID=FIX.5.0<br><br># BeginString: FIX 4.2 over FIXT 1.1<br>DefaultApplVerID=FIX.4.2<br>``` | String | No default. Required for FIXT 1.1 |
