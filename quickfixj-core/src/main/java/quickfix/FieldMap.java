...
public abstract class FieldMap implements Serializable {
  ...
  protected FieldMap(int[] fieldOrder) {
    ...
    fields =
        new TreeMap<>(fieldOrder != null ? new FieldOrderComparator() : null);
  }

  protected FieldMap() { this(null); }
  ...
  private class FieldOrderComparator
    ...
    private int rank(int field, int[] fieldOrder) {
      ...
      return index > -1 ? index
                        : Integer.MAX_VALUE; // unspecified fields are last
    }
  ...
  public void setDouble(int field, double value) { setDouble(field, value, 0); }
  ...
  public void setUtcTimeStamp(int field, LocalDateTime value,
    ...
    setField(new StringField(field,
                             UtcTimestampConverter.convert(
                                 value, includeMilliseconds
  ...
  public void setUtcTimeStamp(int field, LocalDateTime value,
    ...
    setField(new StringField(field,
                             UtcTimestampConverter.convert(value, precision)));
  }
  ...
  public void setUtcTimeOnly(int field, LocalTime value,
                             UtcTimestampPrecision precision) {
    setField(
        new StringField(field, UtcTimeOnlyConverter.convert(value, precision)));
  }
  ...
  public void setField(int key, Field<?> field) { fields.put(key, field); }
  ...
  private FieldException newIncorrectDataException(FieldConvertError e,
                                                   int tag) {
    return new FieldException(
        SessionRejectReason.INCORRECT_DATA_FORMAT_FOR_VALUE, e.getMessage(),
        tag);
  }

  public boolean isSetField(int field) { return fields.containsKey(field); }

  public boolean isSetField(Field<?> field) {
    return isSetField(field.getField());
  }

  public void removeField(int field) { fields.remove(field); }

  public Iterator<Field<?>> iterator() { return fields.values().iterator(); }
  ...
  private boolean isGroupField(int field) { return groups.containsKey(field); }
  ...
  protected void calculateString(StringBuilder buffer, int[] preFields,
                                 int[] postFields) {
    if (preFields != null) {
  ...
  private static final boolean IS_STRING_EQUIVALENT =
      CharsetSupport.isStringEquivalent(CharsetSupport.getCharsetInstance());
    ...
    for (Entry<Integer, List<Group>> entry : groups.entrySet()) {
      ...
      if (!groupList.isEmpty()) {
        if (IS_STRING_EQUIVALENT) {
          result += getStringLength(entry.getKey()) +
                    getStringLength(groupList.size()) + 2;
        } else {
   ...
   */
  public int getGroupCount(int tag) { return getGroups(tag).size(); }

  public Iterator<Integer> groupKeyIterator() {
    return groups.keySet().iterator();
  }

  Map<Integer, List<Group>> getGroups() { return groups; }
  ...
  public boolean hasGroup(int field) { return groups.containsKey(field); }
