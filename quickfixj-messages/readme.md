...
In the QuikFIX/J build, implementations for FIX versions from **FIX4.0 to FIX5.0sp2** are generated from the QuickFIX dictionaries for the specific version. 
...
This scheme works well though in a few cases in which the data type of the Field, with the same designated java class name, has changed between legacy versions.
In these cases a choice needs to be made which data type to use. 
The QuickFIX/J project build prefers the most recent specification of the field which will overwrite the Field generated from the obsolete specification. 
In the unlikely event that a legacy version of a Field is required a custom build can be implemented. 
Please note that a legacy implementation of a Field cannot deterministically be used in the same Java runtime as an incompatible class of the same name.
...
The packaging is done in reverse order so that fields not present in older protocol versions can be pruned prior to packaging. 
...
There is also an orchestration available for FIX4.2 and FIX4.4, the QuickFIX/J project does not employ these resources.
QuickFIX/J uses the legacy FIX Dictionaries for greater legacy compatibility. 
...
The artefacts created by the ```quickfij-messages``` module are <u>**test**</u> dependendencies of ```quickfixj-core```. 
They are not specified as _runtime_ dependencies. This is intended to make it easier to customise QuickFIX/J deployments. 
...
**Please note**: if following the conventions of the QuickFIX/J build, applications using FIX Protocol verisions 5.0 and later will also depend on a ```quickfixj-messages-fixt11``` jar. 
...
Use the same version of code and dictionary generators that are used by the QuickFIX/J version on which the custom build depends.
...
* Modules and directories for FIX Protocol versions that are not required can be removed. The ```quickfixj-messages-all``` contains the code generation. If removing unused FIX protocol versions then this module will need to be edited to omit these versions.
